...
class OTP:
	...
	def applyFunc(self, msg, func, length = None, pad = None, delete = True):
		if length is None:
			if self._length:
				length = self._length
			else:
	...
	def macEncrypt(self, msg, length = None, pad = None):
		...
		return self.encrypt(msg + mac, length * 2, pad)		# Uses 2 * length to 4 * length
	...
	def send(self, data, flags = 0):
		...
		if not self._socket:
			import socket
			raise socket.error		
		enc = self.macEncrypt(data)
		return self._socket.send(str(enc), flags)
	...
	def recv(self, bufsize, flags = 0):	
		...
		dec = self.decryptVerify(cipherText)[0]
