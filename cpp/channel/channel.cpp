...
bool Channel::connect_to_server(int port){
  if (socketfd<0){
...
bool Channel::start_server(int port){
  if (socketfd<0){
    socketfd=socket(AF_INET, SOCK_STREAM, 0);
    if (socketfd<0){
      return false;
 ...
 Channel Channel::accept_client(){
  int connfd = accept(socketfd, NULL, NULL);
  return Channel(connfd);
}
...
void Channel::set_keys(std::string aes_key, std::string aes_iv, std::string hmac_password, std::string init_nonce){
...
void Channel::generateMyNonce(){
  std::string random = str2hexstr(readRand(16));
  my_nonce = random;
}

//retrieves random nonce
std::string Channel::getMyNonce(){
  return my_nonce;
}

//sets the stored random nonce
void Channel::setTheirNonce(std::string n){
  their_nonce = n;
}

//gets the stored random nonce
std::string Channel::getTheirNonce(){
  return their_nonce;
}

bool Channel::send_data(std::string data){
  if(socketfd>=0){
...
bool Channel::send_hmac(std::string data){
  data += "|"+hmac_hash(hmac_password, data);
  return send_data(data);
}

bool Channel::send_aes(std::string data){
  return send_hmac(str2hexstr(aesEncrypt(data, aes_key, aes_iv)));
}

bool Channel::send_nonce(std::string data){
  generateMyNonce();
  return send_aes(str2hexstr(data+"|"+my_nonce+"|"+their_nonce));
}

bool Channel::send_encrypted(std::string data){
  return send_nonce(str2hexstr(data));
}

std::string Channel::recv_data(int num_bytes){
...
std::string Channel::recv_hmac(int num_bytes){
...
std::string Channel::recv_aes(int num_bytes){
...
std::string Channel::recv_nonce(int num_bytes){
  std::vector<std::string> data;
...
std::string Channel::recv_encrypted(int num_bytes){
